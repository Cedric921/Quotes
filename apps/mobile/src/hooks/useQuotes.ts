...
export const useQuotes = (options: UseQuotesOptions = {}): UseQuotesReturn => {
  ...
  const fetchQuotes = useCallback(
    async (pageNum: number, refresh = false) => {
      ...
      try {
        ...
        const error =
          err instanceof Error ? err : new Error("Failed to fetch quotes");
        setError(error);
  ...
  const likeQuote = useCallback(async (quoteId: string) => {
    try {
      await quotesApi.likeQuote(quoteId);
      console.log("Quote liked:", quoteId);
    } catch (err) {
      const error =
        err instanceof Error ? err : new Error("Failed to like quote");
